...
<template>
  <div v-if="images.length" class="grid grid-cols-3 gap-2">
    <div v-for="img in images" :key="img.id"
      class="relative group rounded-lg overflow-hidden aspect-square bg-slate-800">
      <img :src="imageUrl(img.file_path)" :alt="img.damage_types?.join(', ') || 'Accident photo'"
        class="w-full h-full object-cover" />
      <!-- Damage tags overlay -->
      <div v-if="img.damage_types?.length"
        class="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent px-2 py-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
