"""
Build the claim-level source dataset for Qwen3 SFT.

This script processes raw claim data and prepares it for teacher generation
and downstream SFT tasks. It's adapted from gemma4 for the qwen3 pipeline.
"""
...
# Import from the transformer module in the parent directory
try:
    from transformer.dataset_cleaning import DEFAULT_DATASET_PATHS, load_and_clean_claims, print_cleaning_report
except ImportError:
    print("Warning: Could not import from transformer.dataset_cleaning")
    print("Make sure the transformer module is available in the parent directory.")
    # Fallback definitions
    DEFAULT_DATASET_PATHS = [Path("/Users/fzuin/nlp-dataset/dataset_insurance.json")]
    
    def load_and_clean_claims(paths):
        claims = []
        for path in paths:
            if path.exists():
        ...
        stats = {"total_claims": len(claims), "cleaned": 0, "removed": 0}
        return claims, stats
    
    def print_cleaning_report(stats):
        print(f"Cleaning stats: {stats}")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the claim-level source dataset for Qwen3 SFT.")
